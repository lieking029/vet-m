          ...
          <v-card flat class="p-4" style="">
                  ...
                  <v-row v-for="(val, key) in multifield" :key="key">
                    <v-col class="py-1 d-none">
                      <v-text-field
                        ...
                        hide-details=""
                      >
                        <template slot="label"> Field Name </template>
                      </v-text-field>
                    </v-col>
            ...
            <v-row>
              <v-col>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn class="primary" large @click="save">update</v-btn>
                </v-card-actions>
              </v-col>
            </v-row>
          </v-card>
      ...
      <v-card id="dialog" style="background-color: #dadcdc">
        ...
        <v-card class="p-4">
          ...
          <v-card-actions>
            ...
            <v-btn
              class="pull-right"
              @click="AddField(multifield.length)"
              color="primary"
              small
...
export default {
  ...
  methods: {
    ...
    AddField(id) { 
      console.log(this.form.multifield_name[id])
      ...
      if (this.$route.query.p) {
        ...
        await axios
          ...
          .then((result) => {
            ...
            if (result.data.multifield_name) {
              this.form.multifield_name = JSON.parse(
                result.data.multifield_name
              );
              this.form.multifield_value = JSON.parse(
                result.data.multifield_value
              );

              if (result.data.multifield_name.length > 0) {
                for (
                  let index = 0;
                  index < result.data.multifield_name.length ;
                  index++
                ) {
                  this.multifield.push({});
                }
